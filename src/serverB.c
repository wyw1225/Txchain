...
#define B_PORT "22965"  // the port users will be connecting to
...
int main(void)
{
  ...
  int numbytes;
	...
	if ((rv = getaddrinfo(NULL, B_PORT, &hints, &servinfo)) != 0) {
  ...
  if (p == NULL)  {
	...
	//printf("serverB: waiting for connections...\n");
	printf("The ServerB is up and running using UDP on port 22965.\n");
